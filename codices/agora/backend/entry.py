...
def _manifest_overrides() -> dict:
    ...
    try:
        ...
        ic.print(f"⚠️  Agora: could not read manifest_data overrides: {e}")
        return {}
    overrides = {}
...
def get_config(args: str) -> str:
    ...
    so e.g. ``population_target``, ``beta_proving_days``, or governance
    fees can be tuned per realm without republishing the codex.
    """
...
def seed_justice(args: str) -> str:
    ...
    try:
        ...
        ic.print("⚠️  Agora: backend has no seed_justice_template, skipping court seeding")
