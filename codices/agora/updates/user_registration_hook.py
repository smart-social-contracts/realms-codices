...
try:
    ...
    from ..backend.invoice_currency import invoice_currency, no_treasury_token_error
...
def user_register_posthook(user):
    ...
    verification).
    """
    currency = invoice_currency(_manifest())
    if not currency:
        err = no_treasury_token_error()
        ic.print(f"Registration invoice skipped: {err['error']}")
        return
    ...
    try:
        ...
        ic.print(f"Created registration invoice #{invoice.id} for user {user.id}: "
                 f"{MONTHLY_FEE_CKBTC} {currency} (or {MONTHLY_FEE_AGO} AGO)")
        ...
        ggg.Notification(
            ...
            message=(
                ...
                f"- **Pay your registration invoice** — `{MONTHLY_FEE_CKBTC} {currency}` or `{MONTHLY_FEE_AGO} AGO` from the *Invoices* section below\n"
