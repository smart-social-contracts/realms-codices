...
``data/justice.json`` declares the justice system + court hierarchy and
``data/justice_license.json`` the license authorizing it. Seeding must be
idempotent and quarter-aware: quarter-scoped courts exist on every canister,
capital-scoped courts only on the capital (or a standalone realm).
"""
